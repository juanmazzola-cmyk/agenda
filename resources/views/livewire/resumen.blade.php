<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <button wire:click="$toggle('ocultarMontos')" title="{{ $ocultarMontos ? 'Mostrar montos' : 'Ocultar montos' }}"
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Turnos</p>
            <p class="text-xl font-bold text-gray-800">{{ $kpis['turnos'] }}</p>
        ...
        {{-- Facturado --}}
        <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Facturado</p>
            <p class="text-xl font-bold text-gray-800 truncate">
                @if($ocultarMontos) •••• @else ${{ number_format($kpis['facturado'], 0, ',', '.') }} @endif
            </p>
            <p class="text-xs text-gray-400 mt-1">valor total</p>
        </div>

        {{-- Cobrado --}}
        <div class="bg-white rounded-xl shadow-sm p-4 border border-green-100 bg-green-50/30">
            <p class="text-xs font-semibold text-green-600 uppercase tracking-wide mb-1">Cobrado</p>
            <p class="text-xl font-bold text-green-700 truncate">
                @if($ocultarMontos) •••• @else ${{ number_format($kpis['cobrado'], 0, ',', '.') }} @endif
            </p>
            @if($kpis['facturado'] > 0)
        ...
        {{-- Pendiente --}}
        <div class="bg-white rounded-xl shadow-sm p-4 border border-amber-100 bg-amber-50/30">
            <p class="text-xs font-semibold text-amber-600 uppercase tracking-wide mb-1">Pendiente</p>
            <p class="text-xl font-bold text-amber-700 truncate">
                @if($ocultarMontos) •••• @else ${{ number_format($kpis['pendiente'], 0, ',', '.') }} @endif
            </p>
            @if($kpis['facturado'] > 0)
